...
import UR_ICON from "../../assets/UR_Logo.png";
import {
  AUTHENTICATED_MENUS,
  MENU_TYPE,
  menus_categories,
...
export class SideNavBar extends Component<SideNavBarProps, SideNavBarState> {
  ...
  render() {
    ...
    return (
      <Fragment>
        <div className="fixed w-full md:w-64 bg-white top-16 bottom-0 left-0 right-5 transition overflow-y-auto animate__animated animate__fadeInLeft animate__faster">
            ...
            {menus_categories()
              ...
              .map((item, k) => (
                <div key={k + 1} className="py-0">
                  {item.key === MENU_TYPE.OTHERS &&
                    menus.filter((itm) => itm.menu_type === item.key).length >
                      0 && (
                      <div className="text-gray-400 uppercase text-xs px-5 mb-1 mt-6">
                        {item.title}
                      </div>
                    )}
                  {menus
